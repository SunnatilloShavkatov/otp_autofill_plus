...
class OTPPlugin : FlutterPlugin, MethodCallHandler, PluginRegistry.ActivityResultListener,
        ...
        Identity.getSignInClient(activity!!).getPhoneNumberHintIntent(request)
            .addOnSuccessListener { res ->
                ...
                activity!!.startIntentSenderForResult(
                    request.intentSender, credentialPickerRequest, null, 0, 0, 0
                )
            }
    }
    ...
    private fun listenUserConsent(call: MethodCall, result: Result) {
    ...
    private fun listenRetriever(result: Result) {
        if (activity != null) {
            lastResult = result
    ...
    private fun registerSmsUserConsentBroadcastReceiver() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            this.activity?.registerReceiver(
                smsUserConsentBroadcastReceiver, intentFilter, Context.RECEIVER_EXPORTED
            )
            ...
            this.activity?.registerReceiver(
                smsUserConsentBroadcastReceiver, intentFilter, SmsRetriever.SEND_PERMISSION, null
            )
        }
    }
    ...
    private fun registerSmsRetrieverBroadcastReceiver() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            this.activity?.registerReceiver(
                smsRetrieverBroadcastReceiver, intentFilter, Context.RECEIVER_EXPORTED
            )
        } else if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            this.activity?.registerReceiver(
                smsRetrieverBroadcastReceiver, intentFilter, Context.RECEIVER_EXPORTED
